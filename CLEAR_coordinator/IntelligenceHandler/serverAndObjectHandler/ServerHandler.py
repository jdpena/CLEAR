...
"""
class ServerHandler():
    def __init__(self, intel, imageDimensions, workerUrl) -> None:
        self.intel = intel

        self.objects = intel.objects 
        self.depthMatrix = intel.depth
        # self.depthReady = False
        self.target = intel.target
    ...
    def updateObjects(self, input):
        ...
        for i in input:
            ...
            newObjects.append(ObjectFound(i, self.count, 
    ...
    def sendContext(self, context) :
        ...
        # if context is not self.lastSentContext :
        response = requests.post(url, json={'string': context})
    ...
    def llm_chat_updated(self, message) :
        ...
        if response.status_code == 200:
            self.llmGeneratedInstruction = response.json()['string']
            self.awaitingLLMOutput = False
